...
function GrooveUtils() {
  ...
  // Brand mark shown in the MIDI player's expand-to-editor button. Replaces the
  // upstream GrooveScribe "g" logo with the Fluent Drummer "FD" mark. Uses the
  // brand primary rather than the lighter on-dark accent, because the player it
  // sits in renders on a light background in embeds (GrooveEmbed.html ships no
  // stylesheet of its own).
  function addInLineBrandMarkSVG() {
    return (
      '<svg width="20" height="30" viewBox="0 0 60 90" xmlns="http://www.w3.org/2000/svg">' +
      '<text x="30" y="57" text-anchor="middle" fill="#2563eb" ' +
      'font-family="\'Lato\', Calibri, sans-serif" font-size="32" font-weight="700">FD</text>' +
      '</svg>'
    );
  }
  ...
  root.HTMLForMidiPlayer = function (expandable) {
    ...
    if (expandable)
      newHTML +=
        '       <span title="Expand full screen in Fluent Drummer Scribe" class="midiGSLogo" id="midiGSLogo' +
        root.grooveUtilsUniqueIndex +
        '">' +
        addInLineBrandMarkSVG() +
        '       </span>' +
